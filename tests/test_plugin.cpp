...
KAP_TEST("detection resolves the highest-priority matching plugin")
{
    const std::filesystem::path root = scratch_root("detect-priority");
    write_file(root / "Cargo.toml", "[package]\nname = \"demo\"\n");
    write_file(root / "plugins" / "cmake-cpp" / "plugin.kpl",
               R"(
manifest {
  name = "cmake-cpp"
  version = "1.0.0"
...
detect {
  file_exists "Cargo.toml"
    ...
    KAP_ASSERT_EQ(matches.front().located.name, std::string("cargo-rust"));
    KAP_ASSERT_EQ(matches.front().score, 40);
    KAP_ASSERT(matches.front().matched_files.size() >= 1);

    std::filesystem::remove_all(root);
});

KAP_TEST("detection removes superseded plugins before choosing a winner")
{
    const std::filesystem::path root = scratch_root("detect-supersede");
    write_file(root / "marker.txt", "x");
    write_file(root / "plugins" / "generic" / "plugin.kpl",
               R"(
manifest {
  name = "generic"
  version = "1.0.0"
...
detect {
  file_exists "marker.txt"
    ...
    const auto matches = kap::plugin::detect(root);
    KAP_ASSERT_EQ(matches.size(), static_cast<std::size_t>(1));
    KAP_ASSERT_EQ(matches.front().located.name, std::string("generic"));
